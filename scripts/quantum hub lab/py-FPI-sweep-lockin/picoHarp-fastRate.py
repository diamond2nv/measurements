...
class PicoHarp():
    ...
    def __init__(self, dev_id=0):
        ...
        self.phLib.PH_Initialize(self.dev_id, 2);  # 0 = histogramming mode
    ...
    def loadLib():
        ...
        phLib = ctypes.windll.LoadLibrary('phlib')  # has to be installed first with PicoQuant installer
        ...
        phLib.PH_StartMeas.argtypes = [ctypes.c_int, ctypes.c_int]
        phLib.PH_ReadFiFo.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_uint * 2*10**7), ctypes.c_int, ctypes.POINTER(ctypes.c_int)]
        
        return phLib
    
        
    def getCountRates(self, int_time):
        ...
        if int_time >= 100:
            ...
            buff = (ctypes.c_uint * 20000000)()
            n_counts = ctypes.c_int()
            self.phLib.PH_StartMeas(self.dev_id, int_time)
            time.sleep(int_time/1000.)
            self.phLib.PH_ReadFiFo(self.dev_id, buff, 20000000, ctypes.byref(n_counts))
