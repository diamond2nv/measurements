...
def diff(l):
    ...
    l = np.append(l, 0)
    l = np.roll(l, 1)
    return np.array([l[i] - l[i-1] for i in range(1, len(l))])
...
class VoltOut(daq.Task):
    def __init__(self, channel, gate_term):
        daq.Task.__init__(self)
        self.CreateAOVoltageChan(str(channel), "ao0", -10., 10., daq.DAQmx_Val_Volts,
                                 None)
        self.CfgSampClkTiming(gate_term, 10000, daq.DAQmx_Val_Rising, 
                              daq.DAQmx_Val_FiniteSamps, 10000)
        
    def write(self, voltage):
        self.WriteAnalogF64(10000, 1, 10.,daq.DAQmx_Val_GroupByChannel, voltage, None, None)    
...
#        scanBox.addItem(QtGui.QSpacerItem(10, 10, QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Minimum))
    ...
    def start(self):
        ...
        self.v_step = pl.linspace(self.v_i, self.v_i + self.step_size * (10000 - 1), 10000)#, dtype=int)
        ...
        try:
            self.startSelectedInstrument(self.NI_BOX)
    ...
    def NI_run(self):
        ...
        try:
            ...
            if self.running:
                ...
                self.ctr.StopTask()
                data = diff(self.ctr.data) / self.dt * 1000
